document.addEventListener("DOMContentLoaded", () => {
    ...
    for (let i = 0; i < totalBoxes; i++) {
        ...
        box.className = 'box';
        box.id = `box-${i}`;
        box.textContent = '000000'; // Default text for each box
    ...
    startButton.addEventListener('click', () => {
        const maxValue = parseInt(maxValueInput.value) || 150000; // Get max value from input, default to 150000
        ...
        for (let i = 0; i < totalBoxes; i++) {
            ...
            box.textContent = '000000'; // Reset text to '00000'
        ...
        interval = setInterval(() => {
            for (let i = 0; i < totalBoxes; i++) {
                let randomNum;
                do {
                    randomNum = Math.floor(Math.random() * maxValue) + 1; // Generate random number up to maxValue
    ...
    stopButton.addEventListener('click', () => {
        ...
        while (drawnNumbers.length < winnerCount) {
            let finalNum;
            do {
                finalNum = Math.floor(Math.random() * (parseInt(maxValueInput.value) || 150000)) + 1; // Generate final random number up to 150000
        ...
        for (let i = 0; i < totalBoxes; i++) {
            if (!selectedBoxes.includes(i)) {
                const box = document.getElementById(`box-${i}`);
                box.textContent = '000000'; // Set non-winner boxes to '00000'
